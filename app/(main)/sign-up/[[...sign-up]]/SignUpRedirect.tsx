...
// Static-export build: Clerk sign-up only runs on the live Vercel app, so
// this page just hands off to it, preserving any ?redirect_url= param.
export default function SignUpRedirect() {
  useEffect(() => {
    window.location.replace(`${DYNAMIC_APP_URL}/sign-up${window.location.search}`);
  }, []);
    ...
    <main className="flex-1 bg-[#f8f8f5] py-24 px-6 flex items-center justify-center">
      <div className="max-w-md text-center">
        ...
        <p className="text-gray-600 mb-6">
          Sign up is handled on our secure event portal.
        </p>
        <a
          href={`${DYNAMIC_APP_URL}/sign-up`}
