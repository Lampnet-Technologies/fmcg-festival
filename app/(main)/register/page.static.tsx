...
// Static-export build: registration (Clerk auth, Paystack, Postgres) only
// runs on the live Vercel app, so this page just hands off to it, preserving
// any ?tier= selection from the sponsorship/exhibitor pages.
export default function RegisterRedirectPage() {
  useEffect(() => {
    window.location.replace(`${DYNAMIC_APP_URL}/register${window.location.search}`);
  }, []);
    ...
    <main className="flex-1 bg-[#f8f8f5] py-24 px-6 flex items-center justify-center">
      <div className="max-w-md text-center">
        ...
        <p className="text-gray-600 mb-6">
          Registration is handled on our secure event portal.
        </p>
        <a
